...
int cur_1()
{
  ...
  auto pl = curry<decltype(plus), int,int,int>(plus);
  ...
  auto cf = curry_helper<5, decltype(f),float, int, int,float,char,char>()(f);
  //auto var = cf(f);
  std::cout << f(1,2,6.89,'a','z') << "," << cf(1)(2)(6.89)('a')('z') << std::endl;
  auto cf2 = curry<decltype(f), float, int, int, float, char, char>(f);
